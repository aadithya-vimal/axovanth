...
export default function WorkspacePage() {
  ...
  const [newTask, setNewTask] = useState({ title: "", description: "", priority: "medium" as const, status: "backlog" as const, assigneeId: "", dueDate: "" });
  ...
  if (!tickets || !workspaces || myPermissions === undefined || !members || !kanbanTasks) return (
  ...
  const KanbanColumn = ({ status, label }: { status: string, label: string }) => {
    ...
    return (
      <div className="flex-1 min-w-[280px] md:min-w-[300px] flex flex-col gap-4">
        <div className="flex items-center justify-between px-2">
          <h3 className="text-xs font-bold uppercase tracking-wider text-muted flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${status === 'done' ? 'bg-green-500' : 'bg-blue-500'}`} />
            {label} ({tasks.length})
          </h3>
          ...
          {tasks.map(task => (
            ...
            >
              <div className="flex justify-between items-start mb-2">
                ...
                <button onClick={(e) => { e.stopPropagation(); deleteKanbanTask({ taskId: task._id }); }} className="opacity-0 group-hover:opacity-100 text-muted hover:text-red-500 transition-opacity">
                    <Trash2 className="w-3 h-3" />
                </button>
              </div>
              <h4 className="text-sm font-bold text-foreground mb-3">{task.title}</h4>
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-32 animate-in fade-in duration-700 font-sans h-full">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between border-b border-border pb-8 gap-6">
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
            ...
            >
                <Plus className="w-5 h-5" /> {activeTab === 'kanban' ? 'New Task' : 'New Ticket'}
            </button>
        </div>
      </header>
      ...
      {activeTab === 'kanban' && (
        <div className="flex gap-6 overflow-x-auto pb-6 animate-in fade-in custom-scrollbar">
            <KanbanColumn status="backlog" label="Backlog" />
      ...
      {/* KANBAN TASK DRAWER (Edit/Audit/Chat) */}
      {selectedTask && (
          <div className="fixed inset-0 z-[160] flex items-center justify-end bg-black/40 backdrop-blur-md p-4 transition-all">
             <div className="glass-panel w-full max-w-lg h-full rounded-[32px] shadow-2xl animate-in slide-in-from-right duration-300 border-border bg-background flex flex-col overflow-hidden">
                ...
                <div className="flex border-b border-border px-6 bg-background/50">
                  <button onClick={() => setKanbanTab('chat')} className={`py-4 text-xs font-bold uppercase border-b-2 px-4 ${kanbanTab === 'chat' ? 'border-accent text-foreground' : 'border-transparent text-muted'}`}>Discussion</button>
                  <button onClick={() => setKanbanTab('audit')} className={`py-4 text-xs font-bold uppercase border-b-2 px-4 ${kanbanTab === 'audit' ? 'border-accent text-foreground' : 'border-transparent text-muted'}`}>Audit Log</button>
                </div>
                <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                    <div className="space-y-4">
                        <div><label className="text-[10px] font-bold text-muted uppercase">Title</label><input className="input-field mt-1" value={selectedTask.title} onChange={(e) => updateKanbanDetails({ taskId: selectedTask._id, title: e.target.value })} /></div>
                        <div><label className="text-[10px] font-bold text-muted uppercase">Description</label><textarea className="input-field mt-1 h-24 resize-none" value={selectedTask.description || ""} onChange={(e) => updateKanbanDetails({ taskId: selectedTask._id, description: e.target.value })} /></div>
                        <div className="grid grid-cols-2 gap-4">
                            <div><label className="text-[10px] font-bold text-muted uppercase">Priority</label><select className="input-field mt-1 text-xs bg-background" value={selectedTask.priority} onChange={(e) => updateKanbanDetails({ taskId: selectedTask._id, priority: e.target.value as any })}><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option></select></div>
                            <div><label className="text-[10px] font-bold text-muted uppercase">Assignee</label><select className="input-field mt-1 text-xs bg-background" value={selectedTask.assigneeId || ""} onChange={(e) => updateKanbanDetails({ taskId: selectedTask._id, assigneeId: e.target.value as any })}><option value="">Unassigned</option>{members?.map((m:any) => <option key={m.user._id} value={m.user._id}>{m.user.name}</option>)}</select></div>
                        </div>
                    </div>
                    {kanbanTab === 'chat' && (
                        <div className="space-y-4 pt-4 border-t border-border">
                            <div className="space-y-3">{kanbanComments?.map((c: any) => <div key={c._id} className="p-3 rounded-xl bg-foreground/5 text-xs"><p>{c.content}</p><span className="text-[9px] text-muted opacity-60">{new Date(c._creationTime).toLocaleTimeString()}</span></div>)}<div ref={kanbanScrollRef} /></div>
                            <form onSubmit={handleKanbanComment} className="flex gap-2"><input className="input-field py-2 text-xs" value={kanbanComment} onChange={(e) => setKanbanComment(e.target.value)} placeholder="Type a comment..." /><button type="submit" className="p-2 bg-accent rounded-xl text-white"><Send className="w-3.5 h-3.5" /></button></form>
                        </div>
                    )}
                    {kanbanTab === 'audit' && (
                        <div className="space-y-4 pt-4 border-t border-border">
                            {kanbanEvents?.map((e: any, idx: number) => <div key={idx} className="flex gap-3 text-xs"><div className="w-1.5 h-1.5 rounded-full bg-accent mt-1.5" /><div><p className="font-medium">{e.metadata}</p><p className="text-[10px] text-muted">{e.actor?.name} • {new Date(e._creationTime).toLocaleString()}</p></div></div>)}
                        </div>
                    )}
                </div>
             </div>
      ...
      {/* NEW KANBAN TASK MODAL - WITH PRIORITY & ASSIGNEE */}
      {isTaskModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in">
            <div className="glass-panel max-w-sm w-full p-6 rounded-3xl shadow-2xl border-border bg-background animate-in zoom-in-95">
                <div className="flex justify-between items-center mb-6"><h3 className="font-bold">New Task</h3><button onClick={() => setIsTaskModalOpen(false)}><X className="w-5 h-5 text-muted" /></button></div>
                <div className="space-y-4">
                    <div>
                        <label className="text-[10px] font-bold text-muted uppercase tracking-wider">Title</label>
                        <input className="input-field mt-1" autoFocus value={newTask.title} onChange={(e) => setNewTask({...newTask, title: e.target.value})} placeholder="Title" />
                    </div>
                    <div>
                        <label className="text-[10px] font-bold text-muted uppercase tracking-wider">Description</label>
                        <textarea className="input-field h-20 resize-none mt-1" value={newTask.description} onChange={(e) => setNewTask({...newTask, description: e.target.value})} placeholder="Description" />
                    </div>
                    
                    {/* ADDED: Priority & Assignee Selectors for Creation */}
                    ...
                    <button onClick={handleCreateTask} className="apple-button w-full justify-center mt-2">Create</button>
                </div>
            </div>
        </div>
      )}
